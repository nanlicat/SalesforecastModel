...
def plot_and_loss(eval_model, data_source, epoch):
    ...
    test_data_actual=truth.reshape(1, -1)
    predict_actual = scaler.inverse_transform(test_result.reshape(1, -1))
    test_data_actual = scaler.inverse_transform(test_data_actual)
    # # predict_actual=predict
    # # test_data_actual=ts_data['Value']
    mae = mean_absolute_error(test_data_actual, predict_actual)
    rmse = mean_squared_error(test_data_actual, predict_actual, squared=False)  # 计算MAE和RMSE的百分比
    mae_percent = mae / (test_data_actual.max() - test_data_actual.min()) * 100
    # rmse_percent = rmse / (test_data_actual.max() - test_data_actual.min()) * 100
    #
    print("MAE: %.5f" % mae)
    print("MAE百分比: %.5f" % mae_percent)
    # print("RMSE:  %.5f" % rmse)
    # print("RMSE百分比:  %.5f" % rmse_percent)
    plt.plot(test_result, color="red", label="Predictions")
    plt.plot(truth, color="blue", label="True Values")

    plt.grid(True, which='both')
    plt.axhline(y=0, color='k')
    plt.savefig('ARIMATransformer/transformer-epoch%d.png' % epoch)
    plt.close()
